...
%
%		- DRCols:  Cell row vector of strings. This is an ordered 
%				   list of column names corresponding to each column 
%				   in DRData.
%	
%		- colnames: Cell column vector of strings. This is an ordered 
%					list of column names to parse the data by. Data
%					in DRData will be sorted in order of columns in
%					colnames.
%
%	Outputs:
%
...
[valuelist, whichCol, numuniqs] = getUniqCellCols(DRData, DRCols, colnames);
...
while seqnum <= length(colnames)
    ...
    for k = 1: seqloopcount
		...
		thisCol = whichCol(seqnum);
        for p = 1: seqnum - 1
			if whichCol(p) < whichCol(seqnum)
...
clear colvalues valuelist whichCol numuniqs
